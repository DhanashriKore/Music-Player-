...
// Get the playlist elements
const playlist = document.getElementById('recommended-playlist');
...
document.querySelector('.arrow-left').addEventListener('click', () => {
  activeItem = (activeItem - 1 + playlistItems.length) % playlistItems.length;
  updateActiveItem();
});

document.querySelector('.arrow-right').addEventListener('click', () => {
  activeItem = (activeItem + 1) % playlistItems.length;
  updateActiveItem();
});

// Update the active item and animate the playlist
function updateActiveItem() {
  // Remove active class from all items
  Array.prototype.forEach.call(playlistItems, (item) => {
    item.classList.remove('active');
  });

  // Add active class to the current item
  playlistItems[activeItem].classList.add('active');

  // Animate the playlist
  playlist.style.transform = `translateX(${activeItem * -150}px)`;
}

// Auto-rotate the playlist
setInterval(() => {
  activeItem = (activeItem + 1) % playlistItems.length;
  updateActiveItem();
